...
export default function ScoreBoard({ leaderboard, myPlayerId }) {
  ...
  const getRowStyle = (rank, isMe) => {
    const baseBorder = isMe ? 'border-4 border-indigo-600' : 'border-4 border-black';
    const baseShadow = isMe ? 'shadow-[6px_6px_0px_#6366f1] hover:shadow-[10px_10px_0px_#6366f1]' : '';

    if (rank === 1) {
      return `bg-amber-100 ${baseBorder} ${isMe ? baseShadow : 'shadow-[6px_6px_0px_#ffe800] hover:shadow-[10px_10px_0px_#ffe800]'} transition-all hover:-translate-y-1`;
    }
    if (rank === 2) {
      return `bg-slate-100 ${baseBorder} ${isMe ? baseShadow : 'shadow-[6px_6px_0px_#00c3ff] hover:shadow-[10px_10px_0px_#00c3ff]'} transition-all hover:-translate-y-1`;
    }
    if (rank === 3) {
      return `bg-amber-50 ${baseBorder} ${isMe ? baseShadow : 'shadow-[6px_6px_0px_#ff3b30] hover:shadow-[10px_10px_0px_#ff3b30]'} transition-all hover:-translate-y-1`;
    }
    return `${isMe ? 'bg-indigo-50' : 'bg-white'} ${baseBorder} ${isMe ? baseShadow : 'shadow-[4px_4px_0px_#000] hover:shadow-[6px_6px_0px_#000]'} transition-all hover:-translate-y-1`;
  };
        ...
        {leaderboard.length === 0 ? (
          ...
          leaderboard.map((row, idx) => {
            ...
            return (
              <div 
                key={row.playerId}
                className={`relative flex items-center justify-between p-4 sm:p-5 rounded-2xl transition-all duration-300 transform hover:-translate-y-1 hover:shadow-2xl overflow-hidden backdrop-blur-sm animate-fade-in-up ${getRowStyle(row.rank, isMe)}`}
              ...
              >
                ...
                <div className="flex items-center gap-4 sm:gap-6 relative z-10">
                  ...
                  <div>
                    <h4 className={`font-black text-2xl sm:text-3xl tracking-tight mb-1 flex items-center gap-2 ${row.rank === 1 ? 'text-amber-400' : 'text-black'}`} style={row.rank === 1 ? { WebkitTextStroke: '1px #000', textShadow: '3px 3px 0px #000' } : {}}>
                      {row.playerName}
                      {isMe && (
                        <span className="text-[10px] sm:text-xs font-black bg-indigo-600 text-white border-2 border-black px-2 py-0.5 rounded shadow-[2px_2px_0px_#000] uppercase tracking-wider">
                          You
                        </span>
                      )}
                    </h4>
